    ...
    <Box sx={{ maxWidth: 600, margin: 'auto', position: 'relative' }}>
      ...
      >
        {images.map((image, idx) => (
          <SliderItem key={idx}>
            <img src={image} alt={`Slide ${idx}`} style={{ width: '100%', height: '100%' }} />
          </SliderItem>
        ))}
      </Slider>
      <Button onClick={handlePrev} sx={{ position: 'absolute', top: '50%', left: 10, transform: 'translateY(-50%)' }}>
        Prev
      </Button>
      <Button onClick={handleNext} sx={{ position: 'absolute', top: '50%', right: 10, transform: 'translateY(-50%)' }}>
        Next
      </Button>
      <Typography variant="caption" sx={{ position: 'absolute', bottom: 10, left: '50%', transform: 'translateX(-50%)', color: 'white' }}>
